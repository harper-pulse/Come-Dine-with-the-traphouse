...
export const K = {
  ...
  avatarUsage: `${PREFIX}avatar-usage`,
...
export async function loadAll() {
  const [strings, profiles, hostinfo, cards, photos, avatarUsage] = await pipeline([
    ['MGET', K.config, K.reveal, K.secrets, K.version],
    ['HGETALL', K.profiles],
    ['HGETALL', K.hostinfo],
    ['HGETALL', K.cards],
    ['HGETALL', K.photos],
    ['HGETALL', K.avatarUsage],
  ]);
  ...
  return {
    ...
    avatarUsage: parseHashJson(avatarUsage),
  };
}
...
export function buildInitialEvent(input = {}) {
  ...
  const config = {
    ...
    event: {
      ...
      tagline: 'Every team hosts once. Best dinner wins.',
...
export function mergeTeam(team, profile = {}, { includePrivate = false } = {}) {
  ...
  return {
    ...
    portraitThumb: (profile.portrait?.url && profile.portrait.thumb) || '',
    members: members.map((m) => ({
      id: m.id,
      name: m.name || '',
      avatar: m.avatar === 'custom' && !m.avatarUrl ? '' : m.avatar || '',
      avatarUrl: m.avatarUrl || '',
      avatarThumb: (m.avatarUrl && m.avatarThumb) || '',
...
export function publicPhoto(p) {
  return {
    ...
    thumb: !p.thumb ? '' : p.thumb.access === 'private' ? `/api/photos?id=${encodeURIComponent(p.id)}&size=thumb` : p.thumb.url,
    nightId: p.nightId,
    teamId: p.teamId,
    caption: p.caption || '',
    w: p.w || null,
...
export function buildPublicState(data) {
  const base = {
    v: data.version,
    storage: storageKind(),
    features: { photos: photosEnabled(), aiPortraits: aiMode() !== 'off' },
  };
  const { config } = data;
...
export function cleanProfile(body, existing) {
  const members = (existing.members || []).map((m) => {
    ...
    const allowed = input.avatar === '' || AVATARS.includes(input.avatar) || (input.avatar === 'custom' && m.avatarUrl);
    const avatar = input.avatar === undefined ? m.avatar : allowed ? input.avatar : m.avatar;
    return {
      ...m,
